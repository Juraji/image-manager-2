...
alter table local_directory add constraint fk_local_directory_parent_id foreign key (parent_id) references local_directory (id) on delete restrict on update restrict;

alter table local_meta_data add constraint fk_local_meta_data_hash_id foreign key (hash_id) references hash_data (id) on delete restrict on update restrict;

create index ix_local_meta_data_directory_id on local_meta_data (directory_id);
alter table local_meta_data add constraint fk_local_meta_data_directory_id foreign key (directory_id) references local_directory (id) on delete restrict on update restrict;

alter table pin_meta_data add constraint fk_pin_meta_data_hash_id foreign key (hash_id) references hash_data (id) on delete restrict on update restrict;

create index ix_pin_meta_data_board_id on pin_meta_data (board_id);
alter table pin_meta_data add constraint fk_pin_meta_data_board_id foreign key (board_id) references pinterest_board (id) on delete restrict on update restrict;

create index ix_pinterest_board_parent_id on pinterest_board (parent_id);
alter table pinterest_board add constraint fk_pinterest_board_parent_id foreign key (parent_id) references pinterest_board (id) on delete restrict on update restrict;
